...
export default function EditPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (!name.trim() || !image.trim() || !description.trim() || !Number.isFinite(parsedPrice)) {
    ...
    try {
      ...
      setToast({ visible: true, message: `Product "${data.data.name}" updated.` });
      ...
      <main className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm p-6 space-y-4">
          ...
          <div>
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={4} className="mt-1 block w-full rounded-md border px-3 py-2 bg-white text-gray-900" />
          </div>
